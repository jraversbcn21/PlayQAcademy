...
- **Direct-to-main commits after `1847a5d`** (2026-06-16 session):
  ...
  - `a19d180` feat(istqb): add 3rd Static Testing lesson + backfill module 3 resources — module 3 (`istqb-static-testing`) was the thinnest at 2 lessons; added `istqb-l3-3` "Effective reviews and static analysis in practice" (15 sections: review success factors, choosing a review type, static analysis in CI/CD with quality gates, review-vs-static-analysis table, flashcards `istqb-fc-33/34`, 4 sourced resources). Backfilled `resources` on l3-1/l3-2 (had none); bumped module `estimatedMinutes` 30→48. ISTQB lessons 24→25. All resources verifiable (ISTQB syllabus/Glossary, IEEE 1028, SonarQube incl. Quality Gates, ESLint). Typecheck/lint clean (0/0). See Campus status #2.
  - `5b7008b` fix(nav): surface the Glossary page in the navbar — the `/[lng]/glossary` page existed and worked but **nothing linked to it**, so it was unreachable from the UI. Added a "Glosario"/"Glossary" entry to `NAV_LINKS` (between Curriculum and Playground) rendered in both desktop and mobile menus, with `nav.glossary` keys in the es/en `common.json` locales. Typecheck/lint clean (0/0).
  - `501ca7b` feat(glossary): add ISTQB chapter 6 terms (Tool Support for Testing) — 9 terms (test tool, test/defect management tools, execution & coverage tools, automation, static analysis tool, test data tool, automation ROI) tagged chapter `"6"`, closing the gap where the ISTQB glossary covered only ch. 1-5. `CHAPTER_TITLES["6"]` already existed; glossary page derives chapters dynamically (no page change). Typecheck/lint clean (0/0). See Campus status #2.
...
- **Lessons:** 25 — module 6 (`istqb-tools`) expanded 1→3 in `9078a25`; module 3 (`istqb-static-testing`) expanded 2→3 in `a19d180` (added `istqb-l3-3` "Effective reviews and static analysis in practice"; also backfilled `resources` on l3-1/l3-2 which had none; module `estimatedMinutes` 30→48). All ISTQB lessons now carry sourced `resources`.
...
- **Glossary:** chapters 1-6 now covered in `glossary.ts` (bare keys `"1"`…`"6"`, distinct from QA Fundamentals' `qaf-*` keys). Chapter 6 (Tool Support for Testing) added in `501ca7b` (9 terms); `CHAPTER_TITLES["6"]` already existed and the glossary page derives chapters dynamically (no page change).
