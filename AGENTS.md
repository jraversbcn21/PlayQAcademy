...
- **Exams:** 4 defined (`exam-module-1`, `exam-module-2-3`, `exam-midterm`, `exam-final`). Question banks exist for `m1`-`m5` (`module-1.ts`…`module-5.ts`, each tagged single-module to stay pure for future per-module exams), giving a combined pool of 100 questions across those 5 modules. `exam-module-1` (25 q), `exam-module-2-3` (25 q), and `exam-midterm` (needs m1-m4, 40 q) are all ready. `exam-final` (needs m1-m8, 60 q) remains gated "Coming Soon" via `isExamReady` until m6-m8 banks exist too.
